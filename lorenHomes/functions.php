...
function my_acf_google_map_api( $api ){ //
	$api['key'] = 'AIzaSyBpi03OVT5qd-fLCF7KXY6Vq5ssvI86aGo';
	return $api;
}
...
function my_login_logo() { ?>
    <style type="text/css">
    	body.login{
    		background-color: #425563;/* CARPENTER BLUE */
    	}
        #login h1 a, .login h1 a {
            background-image: url(<?php echo get_stylesheet_directory_uri(); ?>/images/lrnhm_pmc.png);
			height:65px;
			width:320px;
...
//DIRECTORY REDIRECT
function redirect_testimonials_archive() {
    if( is_post_type_archive( 'testimonials' ) ) {
        wp_redirect( home_url( '/' ), 301 );
        exit();
    }
}
add_action( 'template_redirect', 'redirect_testimonials_archive' );


function redirect_neighborhoods_archive() {
    if( is_post_type_archive( 'neighborhoods' ) ) {
        wp_redirect( home_url( '/' ), 301 );
        exit();
    }
}
add_action( 'template_redirect', 'redirect_neighborhoods_archive' );
...
add_theme_support('post-thumbnails', array ('post','work','custom_testimonials'));
add_post_type_support( 'testimonials', 'thumbnail' );
set_post_thumbnail_size( 140, 140, true );
add_action( 'attachments_register', 'my_attachments' );


//REGISTERING CUSTOM TAXONOMIES FOR TESTIMONIAL POST TYPE
function wpdocs_create_testimonial_taxonomies() {
    ...
    $args = array(
        ...
        'query_var'         => true,
        'rewrite'           => array( 'slug' => 'testcategory' ),
    );
 
    register_taxonomy( 'testcategory', array( 'testimonials' ), $args );
}
// hook into the init action and call create_portfolio_taxonomies when it fires
//add_action( 'init', 'wpdocs_create_testimonial_taxonomies', 0 );



//NEIGHBORHOOD CUSTOM POST TYPE
function custom_neighborhoods() {
	$labels = array(
		'name'                => _x( 'Neighborhoods', 'Post Type General Name', 'text_domain' ),
		'singular_name'       => _x( 'Neighborhood', 'Post Type Singular Name', 'text_domain' ),
		...
		'add_new'             => __( 'New Neighborhood', 'text_domain' ),
	...
	$args = array(
		'label'               => __( 'Neighborhood', 'text_domain' ),
		'description'         => __( 'Neighborhood information pages', 'text_domain' ),
		'labels'              => $labels,
		'supports'            => array( 'title', 'editor', 'thumbnail'),
		//'taxonomies'          => array( 'category', 'post_tag' ),
		'hierarchical'        => false,
...
// Hook into the 'init' action
add_action( 'init', 'custom_neighborhoods', 0 );

// Add Thumbnail Support
add_theme_support('post-thumbnails', array ('post','work','custom_neighborhoods'));
add_post_type_support( 'neighborhoods', 'thumbnail' );
set_post_thumbnail_size( 140, 140, true );
add_action( 'attachments_register', 'my_attachments' );


//REGISTERING CUSTOM TAXONOMIES FOR NEIGHBORHOOD POST TYPE
function wpdocs_create_neighborhood_taxonomies() {
	// Add new taxonomy, make it hierarchical (like categories)
    $labels = array(
        'name'              => _x( 'Neighborhood Categories', 'taxonomy general name', 'textdomain' ),
        'singular_name'     => _x( 'Neighborhood Category', 'taxonomy singular name', 'textdomain' ),
        'search_items'      => __( 'Search Neighborhood Categories', 'textdomain' ),
        'all_items'         => __( 'All Neighborhood Categories', 'textdomain' ),
        'parent_item'       => __( 'Parent Neighborhood Category', 'textdomain' ),
        'parent_item_colon' => __( 'Parent Neighborhood Category:', 'textdomain' ),
        'edit_item'         => __( 'Edit Neighborhood Category', 'textdomain' ),
        'update_item'       => __( 'Update Neighborhood Category', 'textdomain' ),
        'add_new_item'      => __( 'Add New Neighborhood Category', 'textdomain' ),
        'new_item_name'     => __( 'New Neighborhood Category Name', 'textdomain' ),
        'menu_name'         => __( 'Neighborhood Category', 'textdomain' ),
    );
    ...
    $args = array(
        ...
        'show_admin_column' => true,
        'query_var'         => true,
        'rewrite'           => array( 'slug' => 'neighcategory' ),
    );
 
    register_taxonomy( 'neighcategory', array( 'neighborhoods','testimonials' ), $args );
}
// hook into the init action and call create_portfolio_taxonomies when it fires
add_action( 'init', 'wpdocs_create_neighborhood_taxonomies', 0 );
...
// Hook into the 'init' action
add_action( 'init', 'custom_teams', 0 );

// Add Thumbnail Support
add_theme_support('post-thumbnails', array ('post','work','custom_teams'));
